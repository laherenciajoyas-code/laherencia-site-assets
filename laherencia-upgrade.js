(function() {
  ...
  onReady(function() {
    ...
    css.textContent = [
      ...
      // — Navigation bar — refined, elegant, spaced sections —
      '.header-nav { background: #3B2A24; border-bottom: 2px solid rgba(139,94,60,0.3); padding: 0; }',
      '.header-nav__menu { gap: 0; justify-content: center !important; }',
      '.header-nav__item { position: relative; }',
      '.header-nav__item + .header-nav__item::before { content: ""; position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 1px; height: 18px; background: rgba(237,228,217,0.25); }',
      '.header-nav__link { font-family: "Raleway", sans-serif !important; font-weight: 500; font-size: 0.72rem !important; text-transform: uppercase; letter-spacing: 0.14em !important; color: #EDE4D9 !important; padding: 0.85rem 1.8rem !important; display: block; transition: all 0.3s ease !important; border-bottom: 2px solid transparent !important; }',
      '.header-nav__link:hover { color: #D6A678 !important; background: rgba(237,228,217,0.08) !important; border-bottom-color: #D6A678 !important; }',
      ...
      '  .header-nav__link { font-size: 0.68rem !important; padding: 0.7rem 1rem !important; letter-spacing: 0.1em !important; }',
      '}'
    ...
    navLinks.forEach(function(link) {
      ...
      if (navRenames[originalText]) {
        link.textContent = navRenames[originalText];
      }
    });

    // Also rename in mobile menu
    var mobileLinks = document.querySelectorAll('#mobile-menu a, .sidebar-accordion a');
    ...
    // -------------------------------------------------------
    // 4. INJECT TRUST BADGES (before footer)
    // -------------------------------------------------------
